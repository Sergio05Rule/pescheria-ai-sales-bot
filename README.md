...
16. **The model must never confirm in prose** — the most dangerous failure mode: on large/complex inputs the AI sometimes replies with a natural-language confirmation ("✅ Registrato! Ho caricato gli acquisti...") *instead of* returning the JSON action, so nothing is written but the user thinks it was. Two defenses: (a) a hard prompt rule forbidding textual confirmations of data operations — only the code confirms; (b) a code-side safety net (`looksLikeFakeConfirmation`) that detects success markers + multiple item lines (kg + €) with no action and warns the user that nothing was saved
17. **Never mask failures in multi-action** — when the AI returns multiple actions, the per-action results drive the reply: all-ok shows the success summary, all-failed shows an error, partial-success lists exactly which operations failed. A top-level success message must never override actual write failures
18. **Telegram's 4096-char limit** — large confirmations (many items + report) exceed Telegram's per-message cap and get rejected with `400 message is too long`, so the data is saved but the user sees no reply. `sendTelegram` splits long text into ≤4000-char chunks on line boundaries, and retries as plain text if Markdown parsing fails
